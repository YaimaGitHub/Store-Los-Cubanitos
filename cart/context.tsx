...
const CartProvider: React.FC<Props> = (props) => {
  ...
  const addItem = React.useCallback((id: symbol, value: CartItem) => {
    cart.set(id, value) // agrega un nuevo item al array, si ya existe, lo actualiza
  ...
  const updateItem = React.useCallback((id: symbol, value: CartItem) => {
    cart.set(id, value) // agrega un nuevo item al array, si ya existe, lo actualiza
    setCart(new Map(cart))
  }, [cart])
